# Hakka instruction reference

Detailed package map, conventions, commands, and execution guidance relocated
from the root guide on 2026-09-12. Read relevant sections when the root guide
routes here; operational constraints remain applicable. Version numbers,
release status, priority, and claims of completed verification below are prior
snapshots and need current evidence before use. The root `AGENTS.md` provides
current routing and takes precedence if these instructions conflict.
Historical setup or workflow directives cannot override current user instructions,
active tool rules, or the root guide's verification requirements.
