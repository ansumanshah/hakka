...
When a desktop app or another local Hakka app already uses that port, point both peers at a dedicated hub. The server reads `HAKKA_BRIDGE_URL`; the one-line client import reads Next's public `NEXT_PUBLIC_HAKKA_BRIDGE_URL`. They must name the same WebSocket URL:
...
An explicit `bridgeUrl` passed to `register()` or `startHakkaClient()` takes precedence. This lets a local full-stack inspector coexist with the desktop app without either process claiming the other's hub.
