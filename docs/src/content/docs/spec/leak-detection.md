...
`hakka-core`'s `NetworkRequest[]`). No native (iOS/Android/RN) equivalent exists yet, and
the Mac app does not render findings yet either: the analyzer is exported and tested, but
wiring it into a desktop surface is still UI work.
