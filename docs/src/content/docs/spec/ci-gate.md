...
| Request/response body VALUES never compared — only JSON *shape* (key names + value types, recursively) | A timestamp, nonce, or generated id inside a body must not cause drift as long as its JSON type is unchanged. A key being added/removed, or a key's type changing, IS drift. |
