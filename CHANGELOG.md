...
- **Transport-error mocks (`failure`) and match-budget mocks (`skipCount`/`stopAfter`)** —
  a mock rule can now simulate a
...
- **Request-initiator stack trace** (`hakka-core` + `hakka-browser`) — shows which app code made each call. Opt-in via a Settings toggle (`setStackCapture`), so it's zero-cost by default; shown as an Initiator section in the request detail.
...
- **`hakka-browser/vite`** — an unplugin-based Vite plugin (with `hakka-browser/webpack` and `hakka-browser/rspack` siblings) that auto-injects the overlay in dev with zero app-code (`plugins: [hakka()]`).
