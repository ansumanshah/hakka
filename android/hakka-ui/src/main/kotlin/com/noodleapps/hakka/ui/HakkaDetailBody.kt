...
internal fun DetailBody(raw: String, contentType: String?) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        if (json) {
            FilterChip(rawMode, { rawMode = !rawMode }, { Text(if (rawMode) "Raw" else "Tree") })
        }
...
private fun findMatches(body: String, query: String): List<Int> {
    ...
    val source = body.lowercase()
    val target = query.lowercase()
    val matches = mutableListOf<Int>()
    var start = 0
    while (start < source.length) {
        val next = source.indexOf(target, start)
        if (next < 0) break
        matches += next
        start = next + target.length
    }
    return matches
}
...
private fun JsonTree(raw: String) {
    ...
    if (root == null) {
        Text("Unable to parse JSON", color = MaterialTheme.colorScheme.error)
        return
    }
...
private fun JsonNode(name: String, value: Any?, depth: Int) {
    ...
    Text(
        ...
        modifier = Modifier
            ...
            .then(if (nested) Modifier.clickable { expanded = !expanded } else Modifier),
        fontFamily = FontFamily.Monospace,
        style = MaterialTheme.typography.bodySmall,
    )
    if (!nested || !expanded) return
    when (value) {
        is JSONObject -> (0 until value.length()).forEach { index ->
            val key = value.keys().asSequence().toList()[index]
