...
/// Native project navigation, active work surface, and contextual inspector.
struct ContentView: View {
        ...
        workspace
            ...
            .toolbar {
                ToolbarItem(placement: .status) {
                    WorkspaceStatusView(compact: false)
                }
                ...
                ToolbarItemGroup(placement: .primaryAction) {
                    ...
                    .disabled(model.selection == nil || model.selection?.usesFullWidthWorkspace == true)
    ...
    private var workspace: some View {
        if model.selection == nil || model.selection?.usesFullWidthWorkspace == true {
            NavigationSplitView {
                sidebar
            } detail: {
                CenterPaneView()
    ...
    private var sidebar: some View {
        SidebarView()
            .navigationSplitViewColumnWidth(min: 200, ideal: 216, max: 320)
    }
    ...
    private var sessionCompareErrorPresented: Binding<Bool> {
        Binding(
            ...
            set: {
                isPresented in if !isPresented {
                    model.sessionCompare.clearError()
                }
            }
        )
    }
    ...
    private var environmentErrorPresented: Binding<Bool> {
        Binding(
            ...
            set: {
                isPresented in if !isPresented {
                    model.environment.lastError = nil
                }
            }
        )
    }
