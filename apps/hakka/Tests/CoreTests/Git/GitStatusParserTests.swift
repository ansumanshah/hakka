...
/// Feeds `GitStatusParser` hand-built `git status --porcelain=v2 --branch`
...
struct GitStatusParserTests {
    @Test func parsesTheCurrentBranchFromTheBranchHeadHeader() {
        let output = "# branch.oid abcdef0123456789\n# branch.head main\n# branch.upstream origin/main\n# branch.ab +0 -0\n"
    ...
    @Test func renamedEntryCapturesBothPathsFromTheTabSeparatedFinalField() {
        let output = "2 R. N... 100644 100644 100644 aaaa bbbb R100 new-name.txt\told-name.txt"
        let status = GitStatusParser.parse(output)
