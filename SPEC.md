...
³⁴ `FolderRunner` (`Sources/Core/Runner/FolderRunner.swift`) runs every request nested under a saved folder in sequence, carrying captures and cookies forward request-to-request (one `CookieJar` shared across the run, so a login response's `Set-Cookie` reaches request 5), and deliberately does not stop at the first failure — every request in the plan is attempted regardless of what happened before it. No other Hakka platform is an API client with saved collections to run.
