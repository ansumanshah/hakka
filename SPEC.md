...
²⁶ The bridge wire protocol carries exactly three frame kinds — `request`, `span`, `control` (`Sources/Server/BridgeWireFrame.swift`'s `BridgeFrameKind`, mirroring `packages/hakka-bridge/src/protocol.ts`). There is no console-log or storage-snapshot frame kind, so a connected device's console output and storage contents genuinely cannot reach the Mac app today — this is not "not built yet," it is blocked on a wire-contract change that does not exist, and one that (per the file's own comment on `BridgeDeviceLabel`) would need to land atomically across TypeScript/Swift/Kotlin with shared fixtures, the same bar every other wire-contract change in this repo clears.
