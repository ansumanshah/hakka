...
resolves `hakka-browser` from the local workspace via `"hakka-browser": "file:../../packages/hakka-browser"` in
...
`packages/hakka-browser`, not `examples/vite-app`) on purpose, so it
