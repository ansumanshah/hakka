...
Build the package first (this demo loads the built bundle from `../dist`, not the TypeScript
source):
...
Then serve the **package root** (`packages/hakka-browser/`), not the `demo/` folder itself. The
page loads `../dist/*` relative to its own URL:

```bash
cd packages/hakka-browser
python3 -m http.server 4173
```

Open `http://localhost:4173/demo/index.html`. Don't open it as a `file://` URL: the module Worker
