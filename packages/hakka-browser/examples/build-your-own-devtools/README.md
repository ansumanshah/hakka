...
```sh
just build-browser   # or: bun run --cwd ../../.. build   (builds hakka-core + hakka-browser dist)
...
`hakka-browser` isn't published to npm yet, so this example depends on it via `file:../..` and
