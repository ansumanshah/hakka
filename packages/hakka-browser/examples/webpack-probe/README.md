...
`hakka-browser` dependency is `file:../..` (a symlink — same pattern
...
nothing. Reproduce it yourself: point `hakka-browser` at `file:../..`,
`npm install`, `npm run build`, and `dist/index.html`'s `<body>` is empty.

## The fix (in `../../src/plugin/factory.ts` and `inject.ts`)
...
mounted into the DOM). Re-run against a symlinked `file:../..` install (this
