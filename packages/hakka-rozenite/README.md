...
| Response mutation      | **Ships a real, working per-request response override today**: edit status/body, `actions.addOverride(url, { status, body })`. This is write capability already in production, not a gap.                                                                                         | **Not yet wired into this plugin** (v0.0.1 is read-only). `hakka-core` already has a full `mockEngine`/`ThrottleEngine` used elsewhere across the Hakka ecosystem (`hakka-react-native`'s Mocks/Throttle panels, `hakka mcp`'s `create_mock` tool): bringing that into this panel over the same `control`-message channel `HakkaBridge`/`hakka mcp` already use is the natural next step, but it isn't built here. Noted as a real gap, not glossed over. | `packages/network-activity-plugin/src/ui/components/OverrideResponse.tsx`                                                          |
