...
This directory is nested under `packages/hakka-node/examples/`, one level deeper than
the root workspace's `packages/*` glob matches, so it is **not** a workspace member.
`npm install` here resolves `hakka-node` the way a real external consumer would, not
